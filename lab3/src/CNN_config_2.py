...
c_layer1 = mx.sym.Convolution(data=data, kernel=(3, 3), num_filter=1000, stride=(2, 2))
...
c_layer2 = mx.sym.Convolution(data=c_layer1_pool, kernel=(3, 3), num_filter=500, stride=(2, 2))
c_layer2_act = mx.sym.Activation(data=c_layer2, act_type="tanh")
c_layer2_pool = mx.sym.Pooling(data=c_layer2_act, pool_type="max", kernel=(2, 2), stride=(1, 1))

fc_input = mx.sym.flatten(data=c_layer1_pool)

fc_layer_1 = mx.sym.FullyConnected(data=fc_input, num_hidden=500)
fc_layer_1_act = mx.sym.Activation(data=fc_layer_1, act_type='tanh')
